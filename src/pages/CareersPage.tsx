          ...
          {values.map((value, index) => {
            ...
            return (
              <Reveal key={value.title} asChild as="div" className="h-full">
                <SpotlightCard
                  as="article"
                  className="group flex h-full flex-col rounded-4xl border border-ast-line bg-ast-surface/60 p-7 transition-[transform,border-color] duration-500 ease-ast-out hover:border-ast-brand/30 motion-safe:hover:-translate-y-1.5"
                >
                  <span
                    aria-hidden
                    className="mb-6 flex h-11 w-11 items-center justify-center rounded-xl border border-ast-line bg-ast-surface-raised text-ast-accent"
                  >
                    <Icon className="h-5 w-5" />
                  </span>
                  <h3 className="font-semibold tracking-tighter2">{value.title}</h3>
                  <p className="mt-2.5 text-sm leading-relaxed text-ast-ink-muted">
                    {value.description}
                  </p>
                </SpotlightCard>
              </Reveal>
